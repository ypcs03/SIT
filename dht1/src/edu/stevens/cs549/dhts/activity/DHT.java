...
//		routing.joinCheck();
	...
	public void checkPredecessor() {
		...
		if (pred != null) {
			try {
				...
				info("CheckPredecessor: Predecessor has failed (id=" + pred.id + ")" + e);
				setPred(null);
			}
		}
	}
	...
	public void addNet(String skey, String v) throws Failed {
		int id = NodeKey(skey);
		System.out.println("Adding key-value: " + id + ": " + skey +"--->" + v);
		//if the key's id is equal to the nodes id then add it locally
		if (info.id == id)
			add(info, skey, v);
		else {
			NodeInfo succ = this.findSuccessor(id);
			System.out.println("Finded successor for addNet: " + succ.id);
			add(succ, skey, v);
		}
	}

...
public class WebClient {
	...
	// TODO remove
	/*
	 * Get closest preceding finger for a key id.
	 */
	public NodeInfo getClosestPrecedingFinger(NodeInfo node, int id) throws DHTBase.Failed {
		...
		info("client getClosestPrecedingFinger(" + fingerPath + ")");
	...
	public String[] get(NodeInfo node, String skey) throws DHTBase.Failed {
		...
		if (response == null || response.getStatus() >= 300) {
			...
			TableRow row = response.readEntity(tableRowType).getValue();
			return row.vals;
		}
	}
...
//		URI putPath = node.addr;
//		TableRow row = new TableRow(skey, new String[] { v });
//		info("client putBindings(" + putPath + ")");
//		Response response = putRequest(putPath, Entity.xml(NodeService.tableRowRep(row)));
		UriBuilder ub = UriBuilder.fromUri(node.addr);
		URI putPath = ub.queryParam("key", skey).queryParam("val", v).build();
		info("client putBindings(" + putPath + ")");
		Response response = putRequest(putPath);
		if (response == null || response.getStatus() >= 300) {
			String s = String.format("Failed to add binding: %s -> %s", skey, v);
			throw new DHTBase.Failed(s);
		}
	}
	...
	public void delete(NodeInfo node, String skey, String v) throws DHTBase.Failed {
		...
		info("client deleteBindings(" + deletePath + ")");

...
public class Server extends UnicastRemoteObject implements IServer {
	...
	private static class GetThread implements Runnable {
		...
		private FileOutputStream targetfile = null;
		
		public GetThread(ServerSocket s, FileOutputStream target){
			dataChan = s;
			targetfile = target;
		}
		...
		public void run() {
			...
			try {
				if(targetfile == null){
					...
					OutputStream fos = xfer.getOutputStream();
					ReadAndWrite(file, fos);
				}else{
					Socket xfer = dataChan.accept();
					InputStream is = xfer.getInputStream();
					ReadAndWrite(is, targetfile);
				}
			} catch (Exception e) {
		...
		if (!valid(file)) {
			...
			try {
				...
				*/
				...
				ReadAndWrite(in, xfer.getOutputStream());
				xfer.close();
			} catch (Exception e) {
				System.out.println("Exception : " + e);
				e.printStackTrace();
	...
	public void put(String file) throws IOException, FileNotFoundException, RemoteException {
		...
		case PASSIVE:
			...
			new Thread(new GetThread(dataChan, osf)).start();
			break;
		default:
			break;
		}

...
public class DHT extends DHTBase implements IDHTResource, IDHTNode, IDHTBackground {
	...
	private NodeInfo getSucc(NodeInfo info) throws Failed {
		...
		if (localInfo.addr.equals(info.addr)) {
			...
			return client.getSuccessor(info);
		}
	}
...
//		routing.joinCheck();
	...
	protected String[] get(NodeInfo n, String k) throws Failed {
		if (n.addr.equals(info.addr)) {
			 ...
			 * TODO: Do the Web service call.
			 */
			String []tmp = client.getKeyValues(n, k);
			return tmp;
		}
	}
	...
	public void join(String uri) throws Failed, Invalid {
		 ...
		 */
		try{
			NodeInfo successor = client.findSuccessor(new URI(uri), info.id);
			setSucc(successor);
			stabilize();
		}catch(Exception e){
	...
	public void listenOn(String key, EventListener listener) throws DHTBase.Failed {
		...
		NodeInfo server = this.findSuccessor(id);
		
		NodeInfo local = this.getNodeInfo();
		EventSource eventsource = client.listenForBindings(server, local.id, key);
	...
	public void listenOff(String key) throws DHTBase.Failed {
		...
		this.client.listenOff(server, local.id, key);

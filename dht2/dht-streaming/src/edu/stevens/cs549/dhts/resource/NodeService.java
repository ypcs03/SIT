...
public class NodeService {
	...
	private Response response(String []tmp){
		if(tmp == null)
			return null;
		...
		return Response.ok(str.toString()).header(Time.TIME_STAMP, Time.advanceTime()).build();
	}
	 ...
	 * 
	 */
	public Response getKeyValues(String key){
		try {
			String [] tmp = dht.get(key);			
			return response(tmp);
	...
	public void deleteKeyValue(String key, String val){
		try{
			dht.delete(key, val);
		}catch(Invalid e){
			e.printStackTrace();
	...
	public void setKeyValues(String key, String val){
		try{
			dht.add(key, val);
		}catch(Invalid e){
			e.printStackTrace();
	...
	public EventOutput listenForBinding(int id, String key){

...
class EMFStructure{
	...
	String prod;
	int month;
		double sum_quant_1=0.0;
		double sum_quant_2=0.0;
	}
}
...
/***************Update the emf_struct********************/
				if( record.prod.equals(emf_array.get(i).prod) && record.month == emf_array.get(i).month  ){
					emf_array.get(i).sum_quant_1 += record.quant;
				}
/***************Update the emf_struct********************/
				if( record.prod.equals(emf_array.get(i).prod)  ){
					emf_array.get(i).sum_quant_2 += record.quant;
				}
			}
		}
/********** Applying the having clause ************/
	for(int i = 0; i < emf_array.size(); i ++){
		...
		for(int i = 0; i < emf_array.size(); i ++){
			System.out.println(emf_array.get(i).prod + "\t" + emf_array.get(i).month + "\t" + DF.format(emf_array.get(i).sum_quant_2 - 0.0 < 0.0001 ? -1: emf_array.get(i).sum_quant_1/ emf_array.get(i).sum_quant_2) +"\t" + "");
		}
database.closedb();
	}
}
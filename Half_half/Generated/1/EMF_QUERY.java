...
class EMFStructure{
	...
	String cust;
		...
		int count_avg_2=0;
		double avg_quant_3=0.0;
		double sum_avg_3=0.0;
		int count_avg_3=0;
	}
}
		 ...
		 boolean exist=false;
			if( record.year == 1997){
			 for(int i=0;i<emf_array.size();i++){
				 if(emf_array.get(i).cust.equals(record.cust)){ //if the combination exists, then update the global aggregation functions
					 exist=true;
					 break;
				 }
			 }
...
/***************Update the emf_struct********************/
				if( record.cust.equals(emf_array.get(i).cust) && record.state.equals("NY")  ){
...
/***************Update the emf_struct********************/
				if( record.cust.equals(emf_array.get(i).cust) && record.state.equals("NJ")  ){
...
/***************Update the emf_struct********************/
				if( record.cust.equals(emf_array.get(i).cust) && record.state.equals("CT")  ){
					emf_array.get(i).count_avg_3 ++;
					emf_array.get(i).sum_avg_3 += record.quant; 
					emf_array.get(i).avg_quant_3 = emf_array.get(i).sum_avg_3/emf_array.get(i).count_avg_3;
				}
			}
		}
/********** Applying the having clause ************/
	for(int i = 0; i < emf_array.size(); i ++){
		if(!(emf_array.get(i).avg_quant_1 > emf_array.get(i).avg_quant_2 && emf_array.get(i).avg_quant_1 > emf_array.get(i).avg_quant_3)){
		...
		for(int i = 0; i < emf_array.size(); i ++){
			System.out.println(emf_array.get(i).cust + "\t" + DF.format(emf_array.get(i).avg_quant_1) +"\t" + DF.format(emf_array.get(i).avg_quant_2) +"\t" + DF.format(emf_array.get(i).avg_quant_3) +"\t" + "");
		}
database.closedb();
	}
}
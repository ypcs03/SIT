...
/***************Update the emf_struct********************/
				if( record.prod.equals(emf_array.get(i).prod) && record.month == emf_array.get(i).month-1  ){
...
/***************Update the emf_struct********************/
		...
		while((record = database.NextRecord()) != null){
			...
			for(int i = 0; i < emf_array.size(); i ++){
/***************Update the emf_struct********************/
				if( record.prod.equals(emf_array.get(i).prod) && record.month == emf_array.get(i).month && record.quant >  emf_array.get(i).avg_quant_1 && record.quant <  emf_array.get(i).avg_quant_2  ){
					emf_array.get(i).count_all_3 ++;
				}
			}
		}
/********** Applying the having clause ************/
	for(int i = 0; i < emf_array.size(); i ++){
		...
		for(int i = 0; i < emf_array.size(); i ++){
			System.out.println(emf_array.get(i).prod + "\t" + emf_array.get(i).month + "\t" + emf_array.get(i).count_all_3 + "\t" + "");
		}
database.closedb();
	}
}
	 ...
	 while(record != null){
		 boolean exist=false;
			 for(int i=0;i<emf_array.size();i++){
				 ...
				 && emf_array.get(i).prod.equals(record.prod)){ //if the combination exists, then update the global aggregation functions
					 exist=true;
					 break;
				 }
			 }
...
/***************Update the emf_struct********************/
				if( record.cust.equals(emf_array.get(i).cust) && record.prod.equals(emf_array.get(i).prod)  ){
...
/********** Applying the having clause ************/
	for(int i = 0; i < emf_array.size(); i ++){
		...
		for(int i = 0; i < emf_array.size(); i ++){
			System.out.println(emf_array.get(i).cust + "\t" + emf_array.get(i).prod + "\t" + DF.format(emf_array.get(i).avg_quant_1) +"\t" + DF.format(emf_array.get(i).avg_quant_2) +"\t" + "");
		}
database.closedb();
	}
}